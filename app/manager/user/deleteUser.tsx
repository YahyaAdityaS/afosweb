...
import {ButtonDanger, ButtonSuccess, ButtonWarning,} from "@/components/button";
...
const DeleteUser = ({selectedUser}: {selectedUser: IUser}) => {
    ...
    const [menu, setMenu] = useState<IUser>({...selectedUser})
    const router = useRouter()
    const TOKEN = getCookie("token") || ""
    const openModal = () => {
        setMenu({...selectedUser})
        setIsShow(true)
    }
               ...
               <div className="sticky top-0 bg-sky-600 px-5 pt-5 pb-3 shadow">
                   <div className="w-full flex items-center">
                       <div className="flex flex-col">
                           <strong className="text-white font-bold text-2xl">Delete User</strong>
                           <small className="text-white text-sm">User with existing transaction data cannot be deleted from this page.</small>
                       </div>
                       <div className="ml-auto">
                           <button type="button" className="text-white" onClick={() => setIsShow(false)}>
